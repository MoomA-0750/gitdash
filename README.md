...
- リポジトリの特定ユーザーへの共有（オーナーが管理、共有ユーザーは SHARED セクションに表示）
...
各ベアリポジトリ内に以下のファイルを置くことでメタデータを管理します。

| ファイル | 内容 | 自動作成 |
|----------|------|----------|
| `owner` | オーナーユーザー名（1行） | Web UI 作成時に自動作成 |
| `shared` | 共有ユーザー名（改行区切り） | Web UI 共有設定時に自動作成 |
| `description` | リポジトリ説明文 | Web UI 作成時に自動作成 |

既存リポジトリに手動でオーナーを付与する場合：

```bash
echo "ユーザー名" | sudo tee /var/lib/git/public/リポジトリ名.git/owner

# 例：複数まとめて処理
for repo in /var/lib/git/public/*.git; do
  echo "alice" | sudo tee "$repo/owner"
done
```
